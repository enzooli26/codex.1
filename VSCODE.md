...
构建目录为 `build/`。数据库相对于服务器启动目录创建在 `build/data/charging.db`。

## QML 实时预览

Qt 官方 VS Code 扩展支持 QML Preview。先成功构建 `mobile_client`，然后在命令面板执行 `Qt: Preview changes to QML code live in your application`，选择 `ev_mobile_client.exe`。若扩展找不到资源文件，把 `build` 加入 Qt QML Preview 的 Additional Build Dirs。
