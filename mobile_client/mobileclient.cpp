...
void MobileClient::handle(const QJsonObject &m)
{
    ...
    else if(type==QStringLiteral("charge.completed")){m_chargeStatus=QStringLiteral("目标完成 · %1 kWh · ¥%2").arg(d.value("energy").toDouble(),0,'f',2).arg(d.value("amount").toDouble(),0,'f',2);m_orderId=0;emit chargeChanged();emit notice(QStringLiteral("已达到充电目标并完成结算"),false);refreshStations();refreshOrders();}
}
