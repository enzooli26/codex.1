...
```bash
cp charging-platform/config/app.ini.example charging-platform/config/app.ini
```

编辑 `charging-platform/config/app.ini`。腾讯地图 Key 只填写在本地配置文件中，不提交到代码仓库。数据库路径的父目录必须可写。

## 启动顺序

```bash
./build/server/ev_server --config charging-platform/config/app.ini
...
```bash
sqlite3 build/data/charging.db < charging-platform/database/test_data.sql
```
